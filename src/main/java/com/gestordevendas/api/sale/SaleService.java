...
public class SaleService {
    ...
    public SaleService(SaleRepository repository, SaleItemRepository itemRepository, SalePaymentRepository paymentRepository,
                       SaleNumberService numberService, ProductRepository productRepository, ClientRepository clientRepository,
        ...
        this.repository = repository; this.itemRepository = itemRepository; this.paymentRepository = paymentRepository;
        this.numberService = numberService; this.productRepository = productRepository; this.clientRepository = clientRepository;
        this.priceRepository = priceRepository; this.companyRepository = companyRepository; this.userRepository = userRepository;
        this.stockService = stockService; this.currentUserService = currentUserService; this.tenantContextService = tenantContextService;
    ...
    public List<SaleResponse> list() {
        ...
        return repository.findAllByCompanyIdOrderBySoldAtDesc(context.companyId()).stream().map(s -> response(s, context)).toList();
    }

    @Transactional(readOnly = true)
    public SaleResponse get(UUID id) { TenantContext context = currentTenant(); return response(require(id, context.companyId()), context); }
    ...
    public SaleResponse create(SaleRequest request) {
        ...
        Sale sale = repository.save(Sale.create(company, numberService.next(context.companyId()), client, request.soldAt(), request.paymentType(), user));
        List<SaleItem> items = buildItems(request.items(), context, company, sale, client);
        for (SaleItem item : items.stream().filter(SaleItem::isStockMoved).sorted(Comparator.comparing(i -> i.getProduct().getId())).toList()) {
        ...
        applyTotals(sale, items);
        auditService.record("SALE_CREATED", context.companyId(), context.userId(), "SALE", sale.getId(), null, Map.of("number", sale.getNumber(), "total", sale.getTotal()));
        return response(sale, items, context);
    }

    @Transactional
    public SaleResponse update(UUID id, SaleRequest request) {
        TenantContext context = currentTenant(); tenantGuard.requireOwnerOrAdmin(context); validateUniqueProducts(request.items());
        Sale sale = requireActive(id, context.companyId());
        BigDecimal paid = paid(context.companyId(), id);
        if (sale.getPaymentType() == SalePaymentType.PRAZO && request.paymentType() == SalePaymentType.AVISTA && paid.signum() > 0) {
            throw new ApiException(HttpStatus.CONFLICT, "PAID_SALE_CANNOT_BECOME_CASH", "Venda com pagamentos registrados não pode ser alterada para à vista.");
        }
        Client client = requireClient(request.clientId(), context.companyId());
        Company company = companyRepository.findById(context.companyId()).orElseThrow();
        ...
        if (request.paymentType() == SalePaymentType.PRAZO && newTotal.compareTo(paid) < 0) {
            throw new ApiException(HttpStatus.CONFLICT, "SALE_TOTAL_BELOW_PAID", "O total da venda não pode ser menor que o valor já recebido.");
        }
        applySaleStockDelta(context, oldItems, newItems, id);
        itemRepository.deleteAllByCompanyIdAndSaleId(context.companyId(), id); itemRepository.flush(); itemRepository.saveAll(newItems);
        ...
        auditService.record("SALE_UPDATED", context.companyId(), context.userId(), "SALE", sale.getId(), null, Map.of("number", sale.getNumber(), "total", sale.getTotal()));
    ...
    public SaleResponse cancel(UUID id, SaleCancelRequest request) {
        ...
        sale.cancel(userRepository.findById(context.userId()).orElseThrow(), request.reason());
        auditService.record("SALE_CANCELLED", context.companyId(), context.userId(), "SALE", sale.getId(), request.reason(), Map.of("number", sale.getNumber()));
        return response(sale, items, context);
    }
    ...
    public SaleResponse addPayment(UUID id, PaymentRequest request) {
        TenantContext context = currentTenant(); tenantGuard.requireOwnerOrAdmin(context);
        Sale sale = requireActive(id, context.companyId());
        if (sale.getPaymentType() != SalePaymentType.PRAZO) throw new ApiException(HttpStatus.CONFLICT, "SALE_NOT_CREDIT", "Venda não está a prazo.");
        ...
        Company company = companyRepository.findById(context.companyId()).orElseThrow();
        User user = userRepository.findById(context.userId()).orElseThrow();
        paymentRepository.save(SalePayment.create(company, sale, request.amount(), user));
        auditService.record("SALE_PAYMENT_ADDED", context.companyId(), context.userId(), "SALE", sale.getId(), null, Map.of("amount", request.amount()));
    ...
    public SaleResponse settle(UUID id) {
        ...
        if (outstanding.signum() > 0) {
            Company company = companyRepository.findById(context.companyId()).orElseThrow();
            User user = userRepository.findById(context.userId()).orElseThrow();
            paymentRepository.save(SalePayment.create(company, sale, outstanding, user));
        }
    ...
    public Sale require(UUID id, UUID companyId) {
        return repository.findByIdAndCompanyId(id, companyId).orElseThrow(() -> new ApiException(HttpStatus.NOT_FOUND, "SALE_NOT_FOUND", "Venda não encontrada."));
    }
    public Sale requireActive(UUID id, UUID companyId) {
        Sale sale = require(id, companyId);
        if (sale.getStatus() == SaleStatus.CANCELADA) throw new ApiException(HttpStatus.CONFLICT, "SALE_CANCELLED", "Venda cancelada.");
        return sale;
    }
    public BigDecimal paid(UUID companyId, UUID saleId) { return money(paymentRepository.sumPaid(companyId, saleId)); }
    public BigDecimal outstanding(Sale sale, UUID companyId) {
        if (sale.getStatus() == SaleStatus.CANCELADA || sale.getPaymentType() != SalePaymentType.PRAZO) return money(BigDecimal.ZERO);
    ...
    private void applySaleStockDelta(TenantContext context, List<SaleItem> oldItems, List<SaleItem> newItems, UUID saleId) {
        Map<UUID, BigDecimal> oldQty = stockQuantities(oldItems); Map<UUID, BigDecimal> newQty = stockQuantities(newItems);
        Map<UUID, Product> products = new HashMap<>(); oldItems.forEach(i -> products.put(i.getProduct().getId(), i.getProduct())); newItems.forEach(i -> products.put(i.getProduct().getId(), i.getProduct()));
        Set<UUID> ids = new HashSet<>(oldQty.keySet()); ids.addAll(newQty.keySet());
        for (UUID productId : ids.stream().sorted().toList()) {
            BigDecimal stockDelta = oldQty.getOrDefault(productId, BigDecimal.ZERO).subtract(newQty.getOrDefault(productId, BigDecimal.ZERO));
            if (stockDelta.signum() != 0) stockService.applyDelta(context, products.get(productId), stockDelta,
    ...
    private void applyTotals(Sale sale, List<SaleItem> items) {
        ...
        sale.update(sale.getClient(), sale.getSoldAt(), sale.getPaymentType(), money(total), money(cost));
    }
    ...
    private SaleResponse response(Sale sale, TenantContext context) { return response(sale, itemRepository.findAllByCompanyIdAndSaleId(context.companyId(), sale.getId()), context); }
    private SaleResponse response(Sale sale, List<SaleItem> items, TenantContext context) {
        ...
        BigDecimal paid = paid(context.companyId(), sale.getId()); BigDecimal outstanding = outstanding(sale, context.companyId());
        return new SaleResponse(sale.getId(), sale.getNumber(), sale.getClient() == null ? null : sale.getClient().getId(),
            sale.getClient() == null ? null : sale.getClient().getName(), sale.getSoldAt(), sale.getPaymentType(), sale.getStatus(), sale.getTotal(),
            seeCost ? sale.getCostTotal() : null, seeCost ? sale.getProfitTotal() : null, paid, outstanding, sale.getCancelReason(),
