...
public class PurchaseItem {
    ...
    @ManyToOne(fetch = FetchType.LAZY, optional = false) @JoinColumn(name = "compra_id", nullable = false) private Purchase purchase;
    ...
    public static PurchaseItem create(Company company, Purchase purchase, Product product, BigDecimal quantity, BigDecimal unitCost) {
        PurchaseItem item = new PurchaseItem(); item.id = UUID.randomUUID(); item.company = company; item.purchase = purchase;
        item.product = product; item.quantity = quantity; item.unitCost = unitCost; item.stockMoved = product.isStockControlled(); return item;
    }
